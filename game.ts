...
function drawAll() {
	...
	pos += w * delta/1000;
	pos %= 360;
	let x1 = 1.5 * size + size * Math.cos(pos * Math.PI/180);
	let y1 = 1.5 * size + size * Math.sin(-pos * Math.PI/180); // neg b/c (0, 0) in top left
	context.beginPath();
	...
	let x2 = 3.5 * size + (pos/360) * 5 * size;
	context.beginPath();
